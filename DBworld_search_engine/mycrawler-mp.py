...
class DBworldCrawler:
    ...
    def do_crawl_msg(self, msg):
        corid = current_process().name.split('-')[-1]
        for idx, msg in enumerate(msgs):
            ...
            # save as json
            out_path = "{}/{}-{}.json".format(self.new_dir_path, coreid, idx)
            out_dict = {
                "sent": Sent,
                "type": Type,
                "author": Author,
    ...
    def crawl_menu_url(self):
        ...
        count_per_iteration = total_num/float(ITERATION_COUNT)

        for i in range(0, ITERATION_COUNT):
            list_start = int(count_per_iteration * i)
            list_end = int(count_per_iteration * (i+1))
            print list_start, list_end
            pool.map(self.do_crawl_msg, msgs[list_start:list_end])
